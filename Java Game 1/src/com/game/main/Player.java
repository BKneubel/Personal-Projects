...
public class Player extends GameObject{
	...
	public void tick() {
		...
		if(KeyInput.spaceDown == true && HUD.level >= 20){
	...
	private void collision() {
		for(int i = 0; i < handler.object.size(); i++){
			...
			if(KeyInput.invinc == false && KeyInput.invincibility == false){
				if(tempObject.getId() == ID.BasicEnemy || tempObject.getId() == ID.FastEnemy || tempObject.getId() == ID.SmartEnemy || tempObject.getId() == ID.EnemyBoss){//temp object is now basic enemy
					if(getBounds().intersects(tempObject.getBounds())){
						//collision code
						HUD.HEALTH -= 4;//damage taken
						AudioPlayer.getSound("hit").play();
					}
				}
			}
			else if(KeyInput.invinc == true || KeyInput.invincibility == true){
				if(tempObject.getId() == ID.BasicEnemy || tempObject.getId() == ID.FastEnemy || tempObject.getId() == ID.SmartEnemy || tempObject.getId() == ID.EnemyBoss){//temp object is now basic enemy
					if(getBounds().intersects(tempObject.getBounds())){
						//collision code
						HUD.HEALTH -= 0;//damage taken
					}
				}
			}
